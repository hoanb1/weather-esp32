...
String getDataJson() {
  ...
  if (bmeInitialized) {
    ...
    // Log chỉ khi thay đổi > 0.5°C/% hoặc mỗi 10 giây
    static float lastT = NAN, lastH = NAN, lastP = NAN;
    static unsigned long lastBmeLog = 0;
    if (isnan(lastT) || millis() - lastBmeLog > 10000) {
      addLogf("[DEBUG] BME280 -> Temp: %.1f °C, Humidity: %.1f %%, Pressure: %.2f hPa", t, h, p);
      lastT = t;
      lastH = h;
      lastP = p;
      lastBmeLog = millis();
    ...
    static bool loggedBmeError = false;
    if (!loggedBmeError) {
      addLog("[DEBUG] BME280 not initialized");
      loggedBmeError = true;
    }
  }
  ...
  float correctedGasPPM = mq135->getCorrectedPPM(t, h);

  // Log theo chu kỳ 10 giây
  static unsigned long lastMqLog = 0;
  if (millis() - lastMqLog > 60000) {
    addLogf("[DEBUG] MQ135 -> Raw ADC: %d, Corrected PPM: %.0f", mq_raw, correctedGasPPM);
    lastMqLog = millis();
  }
  ...
  // Log theo chu kỳ 10 giây hoặc khi PM2.5 thay đổi > 5
  static float lastPm25 = NAN;
  static unsigned long lastDustLog = 0;
  if (isnan(lastPm25) ||  millis() - lastDustLog > 60000) {
    addLogf("[DEBUG] Dust -> PM raw: %u, PM2.5: %.1f, AQI: %d", dust, pm25, aqi_pm25);
    lastPm25 = pm25;
    lastDustLog = millis();
  }
  ...
  t = safe_round(t, 1);
  h = safe_round(h, 1);
  correctedGasPPM = safe_round(correctedGasPPM, 0);
  ...
  doc["pm"] = dust;              // PM raw
  doc["mqr"] = mq_raw;           // MQ135 ADC raw
  doc["mqp"] = correctedGasPPM;  // Corrected Air Quality ppm
  doc["aqi"] = aqi_pm25;         // AQI từ PM2.5
  doc["ts"] = ts;                // Timestamp microseconds
